def topFiveMovies():
    #print the first movie
    print("Cars 2")
    
    #print second movie
    print("Cars 3")

    #print third movie
    print("Cars 1")

    #print fourth movie
    print("Bee Movie")

    #print fifth movie
    print("Shrek")

def topFiveSports():

    #print first sport
    print("Football")

    #print second sport
    print("Golf")

    #print third sport
    print("Tennis")

    #print fourth sport
    print("Biking")

    #print fifth sport
    print("Basketball")

def topFiveVideoGames():

    #print first Videogame
    print("Destiny 2")

    #print second Videogame
    print("Escape From Tarkov")
    
    #print third Videogame
    print("Apex Legends")

    #print fourth Videogame
    print("Titanfall 2")

    #print fifth Videogame
    print("Ghost of Tishima")

#definea function that outputs your name
#followed by "is my name"
def mynameis(turtle):


    #output string
    print(turtle + "is my name.")



    #run function
    myNameis("Andrew")
    myNameis("Chipper")
    myNameis("Mark")

#define Peraminter
def timesTwo(chicken):

    #output string
    print(chicken * 2)

def plusFive(pizza):
    print(pizza + 5)

def divideTwo(Pasta):
    print(Pasta / 2)

def madLib(adjective,noun,place,vehicle,pluralnoun,verb,typeOfJob,):

#define function
 def madLib(place,adjective,noun,vehicle,pluralnoun,verb,typeOfJob):
     print("A long long time ago in a galaxy" + noun + "Yoda was down in " + place + "I was stuck in my" + adjective + "I couldent get out of that dang" + vehicle + "of mine so I had to fight to" + place + "While my dad was off" + typeOfJob + "So that is why I ran away from home")

#run madLib
madLib("Coconut", "Apple_Bottom_Jeans", "The_boots_With_The_Fur", :def madLib(place):
     print(place + "food is"+ adjective + "Hello" + place + "Whats your favorite place" + noun + "I dont like that" + pluralnoun + "I like that" +):"Cyber_Truck", Boots, am, Plummer)




print("these are my top five movies")

topFiveMovies()

print("these are my top five sports")

topFiveSports()

print("these are my top five videogames")

topFiveVideoGames()

#run function
timesTwo(30)

#run function
plusFive(72)

#run function
divideTwo(70)
